...
public class BluetoothPulseOximeter extends Activity implements  Button.OnClickListener{
	...
	private final boolean DEFAULT_LEGACY_SMS = false; // use the new format by default
    ...
    @Override
    public void onCreate(Bundle savedInstanceState) {Toast.makeText(getApplicationContext(), "Connected to "
...
//		}
		...
		boolean menuKey = ViewConfiguration.get(getApplicationContext()).hasPermanentMenuKey();
		//if (D) Log.w(TAG, "device has a permanent menu key "+menuKey);
		//if (D) Toast.makeText(getApplicationContext(), "Device has a permanent meny key "+menuKey, Toast.LENGTH_SHORT).show();
		if (menuKey) settingsBtn.setVisibility(View.GONE);
		else settingsBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View arg0) {
				//TODO: redirect user to the settings view
				Intent intent = new Intent().setClass(BluetoothPulseOximeter.this, MenuSettingsActivity.class);
				BluetoothPulseOximeter.this.startActivityForResult(intent, 0);
			}
		});
...
//        editTextPort.setText(bposettings.getString("destination_port", "12345"));
		...
		buttonSms.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
		    	...
		    	if (bposettings!=null) {
		    		...
		    		String msg = "";
			    	//construcSMS(int weight, int systolic, int diastolic, int pulse, int spo2, String measurementDate)
			    	//passing -1 as value for weight, systolic and diastolic
			    	//passing null as value for measurementDate
			    	if (legacySMS) {
			    		msg = constructLegacySMS(PREF_PO, -1, -1, -1, HR, SPO2, null);
			    	} else { 
			    		msg = constructSMS(PREF_PO, -1, -1, -1, HR, SPO2, null);
			    	}
			    	
			        if (phoneNo.length()>0)   {
			        	sendSMS(phoneNo, msg);   
			        } else {
			        	Toast.makeText(getBaseContext(), "Please enter doctor's phone number in 'Settings' menu.", Toast.LENGTH_SHORT).show();
			    	}
		    	}
    ...
    void SmsHrSpo2Values(){
    	...
    	//passing -1 as value for weight, systolic and diastolic
    	//passing null as value for measurementDate
    	String msg = constructSMS(PREF_PO, -1, -1, -1, HR, SPO2, null);
        if (phoneNo.length()>0)   {
        	sendSMS(phoneNo, msg);   
        } else {
        	Toast.makeText(getBaseContext(), "Please enter doctor's phone number in 'Settings' menu.", Toast.LENGTH_SHORT).show();
    ...
    private String constructSMS(String pref,int weight, int systolic, int diastolic, int pulse, int spo2, String measurementDate) {
    	...
    	// temporarily ignoring the SharedPreference passed as parameter, use the defaulsharedpreference for consistency
    	SharedPreferences tmp = PreferenceManager.getDefaultSharedPreferences(this);
    	if (tmp!=null) {
	        ...
	        case REQUEST_CONNECT_DEVICE:
	        	...
	        	//if (Integer.parseInt(bposettings.getString("selected_input_source", "1")) == PREF_INPUT_SRC_BLUETOOTH){
	        		...
	        		if (resultCode == Activity.RESULT_OK) {//1 device is discovered - pair or not pair?
	        			...
	        			if (D) Log.i(TAG, "MAC address of the PO is "+address+" as received by BluetoothPulsOximer activity");
	        			
	        			//Store the MAC address in application's shared preference
	        			bposettingseditor.putString(PREF_MAC_ADDR, address);
	        			bposettingseditor.commit();
	        			Log.w(TAG, "mac address stored in app's shared preferences correctly "+bposettings.getString(PREF_MAC_ADDR, DEFAULT_MAC_ADDR));
